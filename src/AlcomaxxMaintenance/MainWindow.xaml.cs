...
public partial class MainWindow : Window
{
    ...
    private void Resume_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var job = _workspace.LoadJob(dialog.FileName);
            SatNumber.Text = job.SatNumber;
            ClientName.Text = job.ClientName;
            Technician.SelectedItem = job.Technician;
            new ApplicationInventoryWindow(job, _workspace) { Owner = this }.ShowDialog();
        }
        catch (Exception ex)
        {
            MessageBox.Show($"No se pudo reanudar el trabajo:\n{ex.Message}", "ALCOMAXX", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
}
